...
function update(){
  ...
  if(bricks.every(b=>!b.alive)){
    running=false;won=true;over=true;onGameEnd(user,score).then(s=>s&&renderLeaderboard(s,user))
  }
}
...
function draw(){
  ...
  for(const b of bricks){
    if(!b.alive)continue
    ctx.save();ctx.globalAlpha=.85;ctx.drawImage(logoImg,b.x,b.y,BRICKW,BRICKH);ctx.restore()
  }
  // Paddle = char
  ...
  ctx.fillStyle='rgba(130,140,150,.7)';ctx.font='bold 12px Segoe UI';ctx.textAlign='left';ctx.fillText('← → / A D  mover',10,20)
  if(over){
...
let touchX=null
canvas.addEventListener('touchmove',e=>{e.preventDefault();const r=canvas.getBoundingClientRect();const tx=e.touches[0].clientX-r.left;pad.x=Math.max(0,Math.min(GW-PAD_W,(tx/r.width*GW)-PAD_W/2))},{passive:false})
